 ...
 * @author ZLk
 */
public class Crc16JiaoYanMa {

	private static final int[] CRC16Table = { 0x0000, 0x1021, 0x2042, 0x3063, 0x4084, 0x50A5, 0x60C6, 0x70E7, 0x8108,
	...
	public static short CRC16Code(byte[] pcrc, int length) {
		short crc16 = 0;
		short item = 0;
		short crc_h = 0;
		for (int i = 0; i < length; ++i) {
			crc_h = (short) (crc16 >> 8);
			item = (short) (crc_h ^ pcrc[i]);
			crc16 = (short) (crc16 << 8 ^ CRC16Table[item]);
		}
		return crc16;
	}

	public static short CRC16Code(byte[] pcrc, int index, int length) {
		short crc16 = 0;
		short item = 0;
		short crc_h = 0;
		for (int i = 0; i < length; ++i) {
			crc_h = (short) (crc16 >> 8);
			item = (short) (crc_h ^ pcrc[i + index]);
			if (item < 0)
				item += 256;
			crc16 = (short) (crc16 << 8 ^ CRC16Table[item]);
		}
